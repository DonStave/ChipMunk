...
library GenericLogic {
    ...
    function calculateNftDebtData(
        ...
        (, vars.totalDebtInReserve) = ILoanPool(loanAddress)
    ...
    struct CalcLiquidatePriceLocalVars {
        ...
        uint256 nftHighestPriceInETH;
        uint256 nftHighestPriceInReserve;
    ...
    )
    {
        ...
        (, vars.borrowAmount) = ILoanPool(poolLoan).getLoanReserveBorrowAmount(
            loanId
        );

        (vars.ltv, vars.liquidationThreshold,,) = nftData
        .configuration
        .getCollateralParams();

        vars.nftPriceInETH = INFTOracleGetter(nftOracle).getAssetPrice(nftAsset);
        (vars.nftHighestPriceInETH) = INFTOracleGetter(nftOracle).getHighestPrice(nftAsset);
        ...
        vars.nftHighestPriceInReserve = ((10 ** vars.reserveDecimals) * vars.nftHighestPriceInETH) / vars.reservePriceInETH;

        vars.thresholdPrice = vars.nftPriceInReserve.percentMul(vars.liquidationThreshold);
        vars.highestThresholdPrice = vars.nftHighestPriceInReserve.percentMul(vars.liquidationThreshold);
        vars.liquidatePrice = vars.thresholdPrice + vars.thresholdPrice.percentMul(Math.ONE_PERCENT);

        return (vars.borrowAmount, vars.thresholdPrice, vars.liquidatePrice, vars.highestThresholdPrice);
    }
