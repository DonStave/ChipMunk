...
library ReserveLogic {
    ...
    function cumulateToLiquidityIndex(
        ...
        uint256 result = amountToLiquidityRatio + (Math.ray());
    ...
    function init(
        ...
        reserve.liquidityIndex = uint128(Math.ray());
        reserve.variableBorrowIndex = uint128(Math.ray());
    ...
    function _mintToTreasury(
        ...
        if (vars.amountToMint != 0) {
            IChipToken(reserve.cTokenAddress).mintToTreasury(
                vars.amountToMint,
                newLiquidityIndex
            );
        }
