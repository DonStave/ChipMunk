 ...
 * point of utilization and another from that one to 100% 利率模型基于 2 个斜率，一个在“OPTIMAL_UTILIZATION_RATE”利用点之前，另一个从那个到 100%
 * @author ChipMunk
 **/
contract InterestRate is IInterestRate {
    ...
    /**
     * @dev this constant represents the utilization rate at which the pool aims to obtain most competitive borrow rates. 该常数代表资金池旨在获得最具竞争力的借款利率的利用率。
     * Expressed in ray
     **/
     ...
     * Expressed in ray该常数表示高于最优值的超额利用率。 它总是等于
     * 1-最优利用率。 在此处添加为常量以进行气体优化。
     * 以射线表示
     **/

    uint256 public immutable EXCESS_UTILIZATION_RATE;

    // Base variable borrow rate when Utilization rate = 0. Expressed in ray Utilization rate = 0 时的基本变量借用率。以射线表示
    uint256 internal immutable _baseVariableBorrowRate;

    // Slope of the variable interest curve when utilization rate > 0 and <= OPTIMAL_UTILIZATION_RATE. Expressed in ray 当利用率 > 0 且 <= OPTIMAL_UTILIZATION_RATE 时可变利率曲线的斜率。 以射线表示
    uint256 internal immutable _variableRateSlope1;

    // Slope of the variable interest curve when utilization rate > OPTIMAL_UTILIZATION_RATE. Expressed in ray 当利用率 > OPTIMAL_UTILIZATION_RATE 时可变利率曲线的斜率。 以射线表示
    ...
    constructor(
        ...
        EXCESS_UTILIZATION_RATE = Math.ray() - (optimalUtilizationRate_);
    ...
    function calculateInterestRates(
        ...
        vars.currentLiquidityRate = _getOverallBorrowRate(
            ...
            vars.currentVariableBorrowRate
                ...
                Math.PERCENTAGE_FACTOR - (reserveFactor)
            );
