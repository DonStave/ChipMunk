 ...
 * @dev Main point of interaction with an ChipMunk protocol's market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
...
LendPoolStorageExt
{
    ...
    function initialize(IAddressesProvider provider) public initializer {
        ...
        esm = IESM(_addressesProvider.getESM());
    }
     ...
     * @param depositor The address that will receive the cTokens, same as msg.sender if the user
    ...
    function deposit(
        ...
        require(esm.isSwitchPaused(1), Errors.DEPOSIT_PAUSED);
    ...
    function withdraw(
        ...
        require(esm.isSwitchPaused(2), Errors.WITHDRAW_PAUSED);
        return
     ...
     * @param borrower Address of the user who will receive the loan. Should be the address of the borrower itself
    ...
    function borrow(
        ...
        require(esm.isSwitchPaused(3), Errors.BORROW_PAUSED);
    ...
    function repay(
        ...
        require(esm.isSwitchPaused(4), Errors.REPAY_PAUSED);
        return
    ...
    function auction(
        ...
        require(esm.isSwitchPaused(6), Errors.AUCTION_PAUSED);
    ...
    function liquidate(
        ...
        require(esm.isSwitchPaused(7), Errors.LIQUIDATE_PAUSED);
        return
    ...
    /**
     * @dev Modify the status to be available for auction
     * @param loanId The loan ID of the underlying NFT used as collateral
     **/
    function availableAuction(
        uint256 loanId
    ) external nonReentrant {
        require(
            _addressesProvider.getPoolAdmin() == _msgSender(),
            Errors.CALLER_NOT_POOL_ADMIN
        );
        LiquidateLogic.executeAvailableAuction(
            _addressesProvider,
            _reserves,
            _nfts,
            loanId
    ...
    function getAuctionList(uint256 start, uint256 end) external view override returns (uint256[] memory,uint256[] memory){
        return LiquidateLogic.getAuctionList(_addressesProvider, _reserves, _nfts, start, end);
    }
    ...
    )
    {
        ...
        (ltv, liquidationThreshold,,) = nftData
        .configuration
        .getCollateralParams();
    ...
    )
    {
        ...
        loanId = ILoanPool(_addressesProvider.getLendPoolLoan())
        .getCollateralLoanId(nftAsset, nftTokenId);
        if (loanId == 0) {
            return (0, address(0), 0, 0, 0, 0);
        }

        DataTypes.LoanData memory loan = ILoanPool(
    ...
    {
        ILoanPool poolLoan = ILoanPool(_addressesProvider.getLendPoolLoan());
     ...
     * @param asset The address of the underlying asset of the reserve
     * @param cTokenAddress The address of the bToken that will be assigned to the reserve
    ...
    onlyLendPoolConfigurator
    {
        ...
        ILoanPool(_addressesProvider.getLendPoolLoan()).initNft(
            asset,
            cNftAddress
        );
    }
